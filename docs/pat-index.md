...
| PAT-188 | 方法論 | Edu workflow step 逐步教學文案精簡化標準：procedure/common_mistakes 每個 item 只裝一個獨立念頭、不用「，」「——」串接兩件事；單行硬性上限 30 字（程式化計算，不用眼睛估）；只留功能性語氣，情緒性/安慰性用詞一律拿掉；具體公司名/商業平台須先查證是否真「全國通用」，優先引官方監管機構/法規本身；規則時效性宣稱若查證時已完整生效，改陳述現況不沿用過渡期措辭 |
