...
## PAT-188 [方法論]: Edu workflow step 逐步教學文案的精簡化標準（一item一念頭、無情緒詞、單行≤24字但優先在自然語意斷點拆、避免殘句；只管新增/改寫內容，不溯及既往）

**背景**：2026-08-14 新增 `arrival.ts` Step 3(電費/水費/垃圾費說明)
過程中，Lily 連續給了 6 輪精簡化指令：盡量精簡+逐步教學+全國通用+
符合最新法規 → 保險起見三項都寫但拿掉水費垃圾費多餘辦理流程、改引
BetrKV 法源 → 比照全站撰寫辦法(過程中抓到 Nebenkosten 違反 PAT-37/39
德文首次原則) → 換行以利最快速閱讀(拆分一行塞兩件事的句子) → 單行
上限 30 字(逐行程式化計算字數重排，不是眼睛估) → 只維持功能性用語、
拿掉情緒性用詞(刪「不用擔心來不及」)。每一輪都是對前一輪的進一步
收斂，不是互相矛盾的來回修改。

**教訓（可複用的文案精簡標準，之後新增/改寫任何 Edu step 直接比照，
不用每次重新從頭討論一輪）**：
- `procedure`／`common_mistakes` 這類逐步教學型清單，每個 item 只能
  裝一個獨立念頭，不可用「，」「——」串接兩件事勉強塞進一行
- ~~單行硬性上限 30 字(含標點)，超過就拆~~——**2026-08-14 撤銷**，見
  下方「修正二」：改以「是不是兩個獨立念頭」判斷要不要拆，不是字數
- 一律只留功能性語氣(動作指令、事實陳述、法源依據)，情緒性/安慰性
  用詞(不用擔心、放心、別怕、務必等)一律拿掉，即使原意是善意提醒
- 內容涉及具體公司名/商業平台(如 Verivox、Check24)時，先查證是否
  真的「全國通用」——不是全國提供服務的品牌不該出現在「全國通用」
  框架的內容裡，優先改用官方監管機構(如 Bundesnetzagentur)或法規
  本身(如 BetrKV)當唯一來源，同時避免商業平台的隱性置入/背書疑慮
- 內容涉及規則變動時效性的宣稱(如「2025 年 6 月起⋯」)，查證當下
  若該規則已完整生效，改陳述現況本身，不沿用會顯得過時的過渡期措辭

（本教訓已同步整理進 `docs/content-style-guide.md`「七、逐步教學型
清單格式」，該檔案是給快速查閱用的整理版摘要，本 PAT 條目才是權威
版本，兩處異動需同步）

**修正（2026-08-14，硬性規範）**：「情緒性/安慰性用詞一律拿掉」原本
只列了 4 個範例詞（不用擔心、放心、別怕、務必），這輪重新校驗全站
今日更新內容時，只用這 4 個詞去比對漏抓了 2 個真正違規：拆長句時
自己補上的修辭框架「真正風險是⋯」、以及 Step 3 `meta.timing` 欄位
的勸誘語氣「入住前越早辦越好」（比對全檔案其餘 9 個 timing 欄位皆為
純事實時間點才發現這行是唯一的例外）——兩者都沒有命中那 4 個範例詞，
但一樣違反「只留功能性語氣」的精神。**硬性規範**：
- 那 4 個詞只是範例，不是完整清單；真正的判斷標準是「這句話是不是
  在陳述事實/下指令/引法源，還是在額外做修辭渲染、勸誘、下價值判斷」
- `meta.timing` 這類欄位額外檢查法：跟同檔案其餘 step 的同欄位逐一
  比對語氣是否一致，單一欄位語氣跟其他 9 個明顯不同就是可疑信號
- **這類違規無法靠 `check-content-style.mjs` 的關鍵字比對自動抓**（不
  是特定詞彙，是修辭結構），只能在每次改動 Edu workflow 的
  `procedure`／`common_mistakes`／`meta.timing` 後人工逐行重讀一次，
  跟同檔案其餘平行欄位比對語氣一致性

**修正二（2026-08-14，撤銷「單行硬性上限 30 字」的拆分做法）**：原始
規則「超過 30 字就拆」被機械化執行後，把好幾個完整句子攔腰拆成 2-3
個獨立 item，其中最糟的例子：「全國皆可選：E.ON、RWE、EnBW、
Vattenfall」＋「(四大集團)；其餘依居住地而定」——第二個 item 單獨拿
出來看根本不是一句完整的話，只是被字數上限硬切斷的殘句。Lily 指正：
「不是每一次換行都要列點，一點內同樣可以有多行，請確保文意通順」。

**硬性規範（取代原本的 30 字上限）**：
- 判斷「要不要拆成兩個 item」的標準是**這是不是兩個獨立念頭**，不是
  「字數有沒有超過某個數字」——一個念頭再長，只要讀起來是完整通順的
  一句話，就留在同一個 item 裡，讓它在畫面上自然換行即可，不用為了
  湊行數硬性攔腰截斷
- 唯一要避免的是：(1) 把兩個不相關的獨立念頭用「，」「——」硬塞進
  同一個 item；(2) 單一 item 塞進太多念頭變成一大段密集的段落，失去
  逐步教學清單原本「一步一項」的可掃描性——這兩者才是真正要拆分的
  訊號，不是絕對字數
- **每次寫完一個 item，出聲讀一遍（或想像讀出來）**：讀起來是完整
  通順的一句話就對了；如果讀到一半斷句斷得莫名其妙、或聽起來像半句
  話，就是拆錯地方，不是繼續拆下去，是往回合併
- 2026-08-14 當天新增的 exit.ts 電費解約 step 沒有這個問題（那些
  item 本來就是各自獨立的步驟/事實，不是被拆開的同一句話），可以
  當作「item 之間本來就該分開」的對照範例

**修正三（2026-08-14，重新引入硬字數上限，改為 24 字）**：Lily 隨後
決定改變主意，把字數上限重新加回來，門檻比原本的 30 字更嚴（24 字），
同時要求盡量避免修正二警告過的「殘句」問題。**適用範圍限定當天實際
改過的 step**（arrival.ts Step 3/4/5、exit.ts 新增的電費解約
step）——完整掃過兩檔案後發現另有 44 行超過 24 字，全是 2026-07-11
就存在、當天沒有動過的舊內容，Lily 明確選擇「只改當天碰過的部分，
舊內容維持現狀」，不做全站回溯性修改。

**目前生效的硬性規範**：
- 單行硬上限 24 字（含標點），超過就拆，用 `node -e` 程式化計算
  `.length` 逐行驗證，不用眼睛估算
- 拆分優先在自然語意斷點（逗號、句意轉折處）切，不要切在專有名詞/
  括號註解中間，避免重現修正二那種「(四大集團)；其餘依居住地而定」
  式的無頭殘句——寧可切成 3 個較短但各自完整的片段，不要切成 2 個
  但其中一個讀起來不知所云
- 此規範**只套用於新增/改寫的 step**，不強制回頭修改舊內容（跟
  PAT-189 數值格式標準同一個「不溯及既往」原則）

**修正四（2026-08-14，引號直述句不算功能性語氣；文案編輯前先出草稿）**：

Step 4「簽約前一定要問」原本用第一人稱引號直述句格式（如「你會幫我
開房東確認書嗎？」），文法上沒有情緒詞、沒有修辭渲染，但仍不符合
「只留功能性語氣（動作指令、事實陳述、法源依據）」——引號對話格式
本身是一種敘事/腳本語氣，不是陳述事實或下指令。**硬性規範**：
`procedure`／`common_mistakes` 一律用第三人稱功能性動詞開頭（確認/
提供/記錄/申請等），不用「「你會...嗎？」」這種第一人稱引號直述句，
即使內容完全客觀中立。

**文案編輯流程規範**：修改 Edu workflow 既有教學清單文字時，先列出
修改後的候選文案給 Lily 看過、明確採用哪個版本後才寫入檔案，不要
讀完指示就直接編輯——文案類改動的接受度比程式碼修正更主觀，落地
成本（寫入+commit+PR+CI）比純顯示草稿高，先過一輪確認再動手更省
來回成本。

## PAT-189 [方法論]: 全站數據/數值呈現格式標準（2026-08-14 新增）

**背景**：校驗 arrival.ts 過程中發現既有內容數字格式不一致（「12 週」
與「1週」並存），使用者要求訂出明確標準。抽樣掃過 `src/data/edu/*.ts`
既有數字用法，多數已經是「數字＋半形空格＋中文單位」，取此為標準，
不是憑空發明。

**硬性規範（適用全站，不限 Edu workflow；只約束新增/改寫內容，不
強制回頭改舊內容）**：

| 類型 | 格式 | 範例 |
|---|---|---|
| 日期 | `YYYY-MM-DD`（ISO） | `2026-08-14` |
| 數字區間 | 阿拉伯數字＋`-`相連，不加空格 | `1-2`、`4-8` |
| 數字＋中文單位 | 數字與中文單位間留一個半形空格 | `14 天`、`3 個月`、`5 年` |
| 純數字組合 | 不加空格 | `100%`、`2026-08-14` |
| 金額 | 千分位逗號＋半形空格＋幣別 | `1,000 歐元` |
| 法規條號 | `§`後留一個半形空格再接數字 | `§ 54 BMG` |
